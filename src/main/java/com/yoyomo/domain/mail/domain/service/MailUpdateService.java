...
public class MailUpdateService {
	...
	private TransactWriteItemsEnhancedRequest buildTransactRequest(List<Mail> mails, MailStrategy strategy) {
		...
		mails.forEach(mail -> {
			MailTransformRequest dto = MailTransformRequest.of(mail, strategy.getScheduledTime());
			strategy.apply(dto);

			TransactUpdateItemEnhancedRequest<Mail> updateRequest = TransactUpdateItemEnhancedRequest.builder(
					Mail.class)
				.item(mail)
				.build();
